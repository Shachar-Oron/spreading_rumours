...
class cell():
    ...
    def set_cell_class(self, cell_class):
        self.cell_class = cell_class
...
def initialize_grid(width, height, cell_types):
    sump = 0
    counter1 = input_window.s1 * 10000 * input_window.p
    counter2 = input_window.s2 * 10000 * input_window.p
    counter3 = input_window.s3 * 10000 * input_window.p
    counter4 = input_window.s4 * 10000 * input_window.p
    counter4 = int(counter4)

    counterp = 10000 * input_window.p
    print(counterp)

    grid = [[cell(x, y, 'normal') for y in range(GRID_SIZE)] for x in range(GRID_SIZE)]
    # define the probabilities of the classes from the input window
    for r in range(counter4):
        for x in range(0,57):
            for y in range(GRID_SIZE):
                # create a new cell and append it to the row
                new_cell = cell(x, y, "s4")
                grid[x][y] = new_cell
                grid[x][y].set_is_person(True)
    for r in range(int(10000 - counterp)):
        for x in range(57,77):
            for y in range(GRID_SIZE):
                # create a new cell and append it to the row
                new_cell = cell(x, y, "dead")
                grid[x][y] = new_cell
                grid[x][y].set_is_person(False)

    for r in range(int(counter3)):
        for x in range(77,85):
            for y in range(GRID_SIZE):
                # create a new cell and append it to the row
                new_cell = cell(x, y, "s3")
                grid[x][y] = new_cell
                grid[x][y].set_is_person(True)
    for r in range(int(counter1)):
        for x in range(85,93):
            for y in range(GRID_SIZE):
                # create a new cell and append it to the row
                if grid[x][y].get_cell_class() == 'normal':
                    new_cell = cell(x, y, "s1")
                    grid[x][y] = new_cell
                    grid[x][y].set_is_person(True)
    for r in range(int(counter2)):
        for x in range(93,GRID_SIZE):
            for y in range(GRID_SIZE):
                # create a new cell and append it to the row
                if grid[x][y].get_cell_class() == 'normal':
                    new_cell = cell(x, y, "s2")
                    grid[x][y] = new_cell
                    grid[x][y].set_is_person(True)
    return grid
...
def randomize_grid():
    ...
    while True:
        x = random.randint(86, GRID_SIZE - 1)
        y = random.randint(0, GRID_SIZE - 1)
        if grid[x][y].get_is_person() and grid[x][y].get_cell_class() != "S4":
            grid[x][y].set_bit(1)
            print("x: ", x, "y: ", y)
            print("cell class: ", grid[x][y].get_cell_class())
            break
    return grid
...
def create_png(generations, infected_cells):
    ...
    plt.savefig("infected_cells_over_time_seif_b.png")
